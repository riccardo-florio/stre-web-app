"""
    Local Copy of StreamingCommunity API for Python (08/07/25)
    Serve per sistemare il problema della funzione load (che non funziona perché cerca sul sito senza il suffisso /it)
"""
...
class API:
    ...
    def load(self, content_slug):
        # Costruisce l'URL corretto con /it/
        ...
        try:
            ...
            raise InvalidJSON(content_slug, e, resp) from e
        ...
        trailer_url = (
            ...
            if trailer_info and trailer_info[0].get("youtube_id")
            else None
        )

        correlates = props.get("sliders", [{}])[0].get("titles", []) if props.get("sliders") else []
        correlates_list = correlates[:15]
        ...
        if media_type == "TvSeries":
            ...
            seasons = props["title"].get("seasons", [])
            seasons_count = int(props["title"].get("seasons_count")) if props["title"].get("seasons_count") else None
            ...
            for se in seasons:
                ...
                episodes = se_data["props"]["loadedSeason"].get("episodes", [])
                sid = se["title_id"]
                for ep in episodes:
            ...
            return {
                ...
                "tags": [genre.get("name") for genre in preview_data.get("genres", [])],
        ...
        return {
            ...
            "rating": int(float(score) * 1000) if score else None,
            "tags": [genre.get("name") for genre in preview_data.get("genres", [])],
            "duration": int(props["title"].get("runtime")) if props["title"].get("runtime") else None,
            "trailerUrl": trailer_url,
            "recommendations": correlates_list,
        }
